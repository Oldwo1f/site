...
console.log("FluxController");
    ...
    // console.log($scope.flux.contents[13]);
    $scope.newAnswer= {};
    ...
    $scope.fetchPage =function(){
        ...
        if(!$scope.fetching && $scope.asec==false){

            $scope.fetching=true;
            // setTimeout(function() {
                fluxsService.fetchPage($scope.page).then(function(data) {
                    $scope.page++;
    ...
    $(window).scroll(function() {
       ...
       if($(window).scrollTop() > 1000) {
           // alert("near bottom!");
           $scope.$apply(function() {$scope.backtop = true;})
       }else{
        console.log('else');
        $scope.$apply(function() {$scope.backtop = false;})
    ...
    $scope.addFavoris = function(contentid)
    {
    	console.log('here');
        fluxsService.addFavoris(contentid);
    }
    $scope.back =function(){
    ...
    $scope.blague =function(val){
            var patt = new RegExp('^blague','i');
            if(patt.test(val))
                return true;
            patt = new RegExp('calembour','i');
            if(patt.test(val))
                return true;
            patt = new RegExp('devinette','i');
            if(patt.test(val))
                return true;
            patt = new RegExp('brève','i');
            if(patt.test(val))
                return true;
